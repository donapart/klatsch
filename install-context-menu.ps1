...
# Registry base for file context menu (literal * key)
$regBase = "HKCU\SOFTWARE\Classes\*\shell"
$dirBase = "HKCU\SOFTWARE\Classes\Directory\shell"

$entries = @(
    @{ Name = "KlatschSpeak"; Label = "Mit Klatsch vorlesen"; Action = "speak" },
    @{ Name = "KlatschAsk"; Label = "An Klatsch senden"; Action = "ask" },
    @{ Name = "KlatschSummarize"; Label = "Mit Klatsch zusammenfassen"; Action = "summarize" }
)

if ($Uninstall) {
    Write-Host "=== Klatsch Context Menu -- Uninstall ===" -ForegroundColor Yellow
    foreach ($e in $entries) {
        reg delete "$regBase\$($e.Name)" /f 2>$null
        reg delete "$dirBase\$($e.Name)" /f 2>$null
        Write-Host "  Removed: $($e.Label)" -ForegroundColor Green
    }
    $sendToLink = Join-Path ([Environment]::GetFolderPath("SendTo")) "Klatsch.lnk"
    if (Test-Path $sendToLink) { Remove-Item $sendToLink -Force; Write-Host "  Removed: Send To shortcut" -ForegroundColor Green }
...
foreach ($e in $entries) {
    $cmd = "`"$PythonExe`" `"$SendScript`" $($e.Action) `"%1`""
    $dirCmd = "`"$PythonExe`" `"$SendScript`" $($e.Action) `"%V`""

    # File context menu (uses reg.exe because PowerShell treats * as wildcard)
    reg add "$regBase\$($e.Name)" /ve /d $e.Label /f | Out-Null
    reg add "$regBase\$($e.Name)\command" /ve /d $cmd /f | Out-Null

    # Directory context menu
    reg add "$dirBase\$($e.Name)" /ve /d $e.Label /f | Out-Null
    reg add "$dirBase\$($e.Name)\command" /ve /d $dirCmd /f | Out-Null

    Write-Host "  Installed: $($e.Label)" -ForegroundColor Green
}
